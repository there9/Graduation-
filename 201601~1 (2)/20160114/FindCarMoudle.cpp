...
bool FindCar_NumberModule(IplImage *img)
{
	...
	//image_wrap = cvCreateImage(cvSize(240, 80), 8, 1);
	...
	roiImage = (IplImage*)cvClone(image_capture);
	cvSetImageROI(roiImage, returnCar.RoiImage);
	IplImage* carRoiImage = cvCreateImage(cvSize(returnCar.RoiImage.width, returnCar.RoiImage.height), roiImage->depth, roiImage->nChannels);
	image_gray = cvCreateImage(cvSize(returnCar.RoiImage.width, returnCar.RoiImage.height), 8, 1);
	image_processing = cvCreateImage(cvSize(returnCar.RoiImage.width, returnCar.RoiImage.height), 8, 1);
	image_copy_from_processing = cvCreateImage(cvSize(returnCar.RoiImage.width, returnCar.RoiImage.height), 8, 1);
	
	
	cvCopy(roiImage, carRoiImage, 0);

	
	cvCvtColor(carRoiImage, image_gray, CV_BGR2GRAY);//회색
	...
	inblob.DoLabeling();
	inblob.BlobBigSizeWidthConstraint(image_processing->width / 1.2);
	inblob.BlobBigSizeHeightConstraint(image_processing->height / 2.3);
	inblob.BlobSmallSizeConstraint(30,10);
	...
	for (int j = 0; j < inblob.m_nBlobs; j++)
	{
		...
		//cvDrawRect(carRoiImage, inpt1, inpt2, cvScalar(0, 0, 255), 2);
		...
		inlabeled = cvCreateImage(cvSize(numberRoiImage->width, inblob.m_recBlobs[j].height), 8, 3);
		...
		if (count_inner > 5 && count_inner < 10)
		{
			...
			cvLine(carRoiImage, inpt1, inpt4, CV_RGB(0, 0, 0), 3);
			cvLine(carRoiImage, inpt4, inpt2, CV_RGB(0, 0, 0), 3);
			cvLine(carRoiImage, inpt2, inpt3, CV_RGB(0, 0, 0), 3);
			cvLine(carRoiImage, inpt3, inpt1, CV_RGB(0, 0, 0), 3);
			cvCircle(carRoiImage, inpt1, 5, CV_RGB(255, 0, 0), 2);
			cvCircle(carRoiImage, inpt3, 5, CV_RGB(0, 0, 255), 2);
			cvCircle(carRoiImage, inpt4, 5, CV_RGB(0, 0, 0), 2);
			cvCircle(carRoiImage, inpt2, 5, CV_RGB(255, 255, 255), 2);
			CvFont* font = new CvFont;
			cvInitFont(font, CV_FONT_VECTOR0, 1.0f, 1.0f, 0, 6, 1);
			cvPutText(carRoiImage, returnCar.carNumber.c_str(), cvPoint(10, 30), font, CV_RGB(0, 0, 0));
			isDetect = true;
		}
	}
	cvShowImage("cam", carRoiImage);
	cvReleaseImage(&carRoiImage);
