...
void CBlobLabeling::Blob_X_SmallSort()
{
	for (int i = 0; i < m_nBlobs; i++)
	{
		for (int j = 0; j < m_nBlobs; j++)
		{
			if (m_recBlobs[i].x < m_recBlobs[j].x)
			{
				CvRect tmpRect = m_recBlobs[i];
				m_recBlobs[i] = m_recBlobs[j];
				m_recBlobs[j] = tmpRect;
			}
		}
	}
...
void CBlobLabeling::BlobBigSizeHeightConstraint(int nHeight)
{
	_BlobBigSizeHeightConstraint(nHeight, m_recBlobs, &m_nBlobs);
}
void CBlobLabeling::_BlobBigSizeHeightConstraint(int nHeight, CvRect* rect, int* nRecNumber)
{
	if (*nRecNumber == 0)	return;
...
void CBlobLabeling::BlobBigSizeWidthConstraint(int nWidth)
{
	_BlobBigSizeWidthConstraint(nWidth, m_recBlobs, &m_nBlobs);
}

void CBlobLabeling::_BlobBigSizeWidthConstraint(int nWidth, CvRect* rect, int* nRecNumber)
{
	if (*nRecNumber == 0)	return;
